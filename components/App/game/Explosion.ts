...
export class Explosion {
    ...
    constructor(x: number, y: number, z: number, color: [number, number, number] = [1.0, 0.4, 0.0], direction?: vec3, scaleMultiplier: number = 1.0, type: 'muzzle' | 'normal' | 'grenade' | 'trail' = 'normal') {
        ...
        if (type === 'grenade') {
            ...
            const numSmoke = 65 * scaleMultiplier;
            ...
            for (let i = 0; i < numSmoke; i++) {
                ...
                this.particles.push({ pos, vel, life, maxLife: life, colorIdx: 3 + Math.floor(Math.random() * 2), scaleMultiplier: scaleMultiplier * (2.0 + Math.random()*2.5), type: 'smoke' });
            }
